...
//		$('.image-viewer img').attr('src','images/'+info['file']); //또는 info.file;         info의 typeof 는 object
	...
	_slideShow: function(){
		if(imageViewer._intervalId){
			...
			//슬라이드쇼 중지 상태...
			imageViewer._intervalId = setInterval(imageViewer._changeImage,2000);
			$(this).text('슬라이드 중지');
		}
	},
	init: function(){
...
/*			$('.image-viewer button').click(imageViewer._changeImage);
			$('.image-viewer img').dblclick(imageViewer._consoleImageLog);
			$('.image-viewer img').mousedown(imageViewer._changeImage);*/
